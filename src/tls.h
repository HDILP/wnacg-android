 ...
 *   stack cannot do TLS 1.2. We use BearSSL which DOES modern TLS, but
 *   we intentionally do NOT verify the server certificate chain. This is
 *   vulnerable to active MITM. For a personal downloader on your own
 *   device this is a pragmatic choice; do not use on untrusted networks.
 *
...
/* Returns the last BearSSL error code if tls_connect failed, else 0.
 * Useful for diagnostics (see bearssl_ssl.h BR_ERR_*). */
